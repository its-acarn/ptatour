...
// Get current scores
app.get('/get-scores', (async (req: Request, res: Response<ApiResponse>) => {
  try {
...
app.post('/record-win', (async (req: Request<{}, {}, RecordWinRequest>, res: Response<ApiResponse>) => {
  try {
    ...
    const currentScore = parseInt(String(updatedScores[winnerIndex][1] || 0));
...
app.post('/delete-game', (async (req: Request<{}, {}, DeleteGameRequest>, res: Response<ApiResponse>) => {
  try {
    ...
    // 2. Check if the game index is valid
    if (gameIndex >= resultsData.length) {
      return res.status(400).json({ success: false, message: 'Game index out of range' });
    }
    
    // 3. Get the game record to be deleted
    const gameToDelete = resultsData[gameIndex];
    
    // 4. Get the current winner
    const winner = gameToDelete[1]; // Assuming winner is in the second column
    
    // 5. Update the summary scores
    // Get the current summary scores
    const summaryScores = await index._readGoogleSheet(
      googleSheetClient, 
      index.sheetId, 
      index.summaryTabName, 
      index.summaryRange
    ...
    console.log("summaryScores",summaryScores)
    
    // Find the winner's index in the summary scores
    const winnerIndex = summaryScores.findIndex(row => {
      if (!row[0] || typeof row[0] !== 'string') return false;
      return row[0].toLowerCase() === winner.toLowerCase();
    });
    
    console.log("winnerIndex",winnerIndex)

    if (winnerIndex === -1) {
      return res.status(400).json({ success: false, message: 'Winner not found in summary scores' });
    }
    
    // Create a deep copy of the summary scores
    const updatedScores = JSON.parse(JSON.stringify(summaryScores));
    
    // Decrement the winner's score (in column 1)
    const currentScore = parseInt(String(updatedScores[winnerIndex][1] || 0));
    ...
    try {
      ...
      // This is a more direct way to delete rows in Google Sheets
...
// Reset all scores
app.post('/reset-scores', (async (req: Request, res: Response<ApiResponse>) => {
  try {
    const googleSheetClient = await index._getGoogleSheetClient();
...
// Get game history
app.get('/get-game-history', (async (req: Request, res: Response<ApiResponse>) => {
  try {
    const googleSheetClient = await index._getGoogleSheetClient();
    ...
    const history = resultsData
      .map((row, index) => {
        ...
        if (row.length >= 8) {
          ...
          winnerIndex = storedNames.findIndex(name => {
      ...
      .filter((item): item is NonNullable<typeof item> => item !== null) // Remove null entries (empty rows)
